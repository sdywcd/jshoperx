...
public class SHA1 {
	...
	// 传入byte字节数组，计算sha-1摘要，返回相应的十六进制字符串
	public static String getDigestOfString(byte[] byteData) {
		return byteArrayToHexString(getDigestOfBytes(byteData));
	}

	// 传入字符串，计算sha-1摘要，返回相应的十六进制字符串
	public static String getDigestOfString(String data) {
		return byteArrayToHexString(getDigestOfBytes(data.getBytes()));
	}

	// 传入字符串和用户验证码，计算sha-1摘要，返回相应的十六进制字符串
	public static String getDigestOfString(String data,String accountCode) {
		StringBuffer buffer = new StringBuffer(data).append(accountCode);
		return byteArrayToHexString(getDigestOfBytes(buffer.toString().getBytes()));
	}
