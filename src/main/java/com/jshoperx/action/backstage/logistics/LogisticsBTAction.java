...
public class LogisticsBTAction extends BaseTAction {
	...
	public void processTemplatesetTlist(List<LogisticsBusinessT> list) {
		...
		for (Iterator<LogisticsBusinessT> it = list.iterator(); it.hasNext();) {
			LogisticsBusinessT lb = it.next();
			if (StringUtils.equals(BaseEnums.SupportType.SUPPORT.getState(),
					lb.getInsure())) {
				lb.setInsure(BaseEnums.SupportType.SUPPORT.getName());
			} else {
				lb.setInsure(BaseEnums.SupportType.UNSUPPORT.getName());
			}
			if (StringUtils.equals(BaseEnums.SupportType.SUPPORT.getState(),
					lb.getIsCod())) {
				lb.setIsCod(BaseEnums.SupportType.SUPPORT.getName());
			} else {
				lb.setIsCod(BaseEnums.SupportType.UNSUPPORT.getName());
			}
			lb.setVisible(BaseEnums.DataShowState.getName(lb.getVisible()));
			lb.setState(BaseEnums.LogisticsShowRange.getName(lb.getState()));
			lb.setSendrange(BaseEnums.LogisticsSupportRange.getName(lb.getSendrange()));
	...
	public String findAllLogisticsBusinessTjson() {
		...
		Criterion criterion=Restrictions.eq("state", BaseEnums.LogisticsShowRange.BACKSTAGE.getState());
