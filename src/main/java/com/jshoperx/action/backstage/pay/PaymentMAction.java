package com.jshoperx.action.backstage.pay;
...
public class PaymentMAction extends BaseTAction {
	...
	public void processPayment(List<PaymentM> list) {
		...
		for (Iterator<PaymentM> it = list.iterator(); it.hasNext();) {
			...
			if(StringUtils.equals(BaseEnums.PayMentType.INSTANTINTERFACE.getState(), pm.getPaymentInterface())){
				pm.setPaymentInterface(BaseEnums.PayMentType.INSTANTINTERFACE.getName());
			}
			if(StringUtils.equals(BaseEnums.PayMentType.SECUREDINTERFACE.getState(), pm.getPaymentInterface())){
				pm.setPaymentInterface(BaseEnums.PayMentType.SECUREDINTERFACE.getName());
			}
			if(StringUtils.equals(BaseEnums.PayMentType.STANDARD.getState(), pm.getPaymentInterface())){
				pm.setPaymentInterface(BaseEnums.PayMentType.STANDARD.getName());
			}
			if(StringUtils.equals(BaseEnums.SupportType.SUPPORT.getState(), pm.getIsFast())){
				pm.setIsFast(BaseEnums.SupportType.SUPPORT.getName());
			}
			if(StringUtils.equals(BaseEnums.SupportType.UNSUPPORT.getState(), pm.getIsFast())){
				pm.setIsFast(BaseEnums.SupportType.UNSUPPORT.getName());
			}
	...
	public String openPayment() {
		if (StringUtils.isNotBlank(this.getPaymentid())) {
			...
			for(String s:strs){
				...
				if(pm!=null){
					pm.setState(BaseEnums.DataUsingState.USING.getState());
					this.paymentMService.update(pm);
				}
			}
			this.setSucflag(true);
	...
	public String closePayment() {
		if (StringUtils.isNotBlank(this.getPaymentid())) {
			...
			for(String s:strs){
				PaymentM pm=this.paymentMService.findByPK(PaymentM.class, s);
				if(pm!=null){
					pm.setState(BaseEnums.DataUsingState.UNUSING.getState());
					this.paymentMService.update(pm);
				}
			}
			this.setSucflag(true);
	...
	public String findAllPaymentForbsOrder() {
		Criterion criterion=Restrictions.eq("state", BaseEnums.DataUsingState.USING.getState());
